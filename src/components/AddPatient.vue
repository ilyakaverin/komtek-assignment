<script lang="ts">
import { Patient, calculate_age } from "@/service";
    ...
    log(e: Event): void {
      ...
      .then((errors) => {
        ...
        if (errors.length > 0) {
          ...
          console.log(calculate_age(user.birthdate))
...
<template>
  <div class="container">
    <form>
      <input v-bind:class="[this.errors.includes('surname') ? 'danger' : null]" type="text" placeholder="Фамилия" v-model="surname" required />
      <span class="error" v-if="this.errors.includes('surname')">Ошибка</span>
      <input v-bind:class="[this.errors.includes('name') ? 'danger' : null]"  type="text" placeholder="Имя" v-model="name" />
      <span class="error" v-if="this.errors.includes('name')">Ошибка</span>
      <input v-bind:class="[this.errors.includes('secondName') ? 'danger' : null]" type="text" placeholder="Отчество" v-model="secondName" />
      <span class="error" v-if="this.errors.includes('secondName')">Ошибка</span>
      <input v-bind:class="[this.errors.includes('birthdate') ? 'danger' : null]" type="date" placeholder="Дата рождения" v-model="birthdate" />
      <span class="error" v-if="this.errors.includes('birthdate')">Ошибка</span>
      <select name="sex" id="gender" v-model="gender">
        <option value="Male">Мужчина</option>
        <option value="Female">Женщина</option>
      </select>
      <input v-bind:class="[this.errors.includes('snils') ? 'danger' : null]" type="text" placeholder="Снилс" v-model="snils" />
      <span class="error" v-if="this.errors.includes('snils')">Ошибка</span>
      <div class="input-group">
        <input v-bind:class="[this.errors.includes('weigth') ? 'danger' : null]" type="number" placeholder="Вес" v-model="weight" />
        <span class="error" v-if="this.errors.includes('weight')">Ошибка</span>
        <input v-bind:class="[this.errors.includes('height') ? 'danger' : null]" type="number" placeholder="Рост" v-model="height" />
        <span class="error" v-if="this.errors.includes('height')">Ошибка</span>
        <span class="age">Возраст: {{ calc_age }}</span>
      </div>
      <button @click="action">{{!this.current ? "Отправить" :  "Обновить"}}</button>
    </form>
  </div>
</template>
